...
/*
 * Title                   : St Barth View
 * File                    : application/controllers/user/babysitter.php
 * File Version            : 1.0
 * Author                  : Marius-Cristian Donea
 * Created / Last Modified : 03 July 2011
 * Last Modified By        : Marius-Cristian Donea
 * Description             : Login User - Babysitter Controller.
*/
    ...
    class Babysitter extends CI_Controller{
        ...
        public function add(){
            if ($this->session->userdata('stbartsview-user')){
                ...
                $data['user_add_offer_location_title'] = $this->lang->line('user_add_babysitter_location_title');
                $data['user_add_offer_location_info'] = $this->lang->line('user_add_babysitter_location_info');
                $data['user_add_offer_alternative_address'] = $this->lang->line('user_add_babysitter_alternative_address');
                $data['user_add_offer_details_title'] = $this->lang->line('user_add_babysitter_details_title');
                $data['user_offer_details_name'] = $this->lang->line('user_babysitter_details_name');
                $data['user_offer_details_description'] = $this->lang->line('user_babysitter_details_description');
                ...
                $this->load->view('frontend/user/templates/offers/add-babysitter-template', $data);
            }
            else{
                redirect('user/redirect');
            }
        ...
        public function validateName(){
            if ($this->input->post('name')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_babysitter_details_name_invalid').'</font>';
            }
        }
        ...
        public function validateDescription(){
            if ($this->input->post('description')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_babysitter_details_description_invalid').'</font>';
            }
        }
        ...
        public function item(){
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    $this->userId = $this->session->userdata('stbartsview-user');
                    $this->babysitterId = $this->input->post('id');

                    if ($this->validBabysitterRequest()){
                        $data = $this->lang->language;

                        $babysitter = $this->CI->Offers_model->getData($this->babysitterId, 'all')->row_array(0);

                        $babysitter['short_description'] = $this->CI->Functions_model->shortText($babysitter['description'], 150);
                        $babysitter['first_image'] = $this->CI->Offers_model->getFirstImage($babysitter['id']);
                        $babysitter['no_deals'] = $this->CI->Offers_model->getNoDeals($babysitter['id']);
                        $data['babysitter'] = $babysitter;


                        $this->load->view('frontend/user/templates/offers/babysitters/babysitter-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBabysitterRequest()){
                        ...
                        $details = $this->CI->Offers_model->getData($this->babysitterId, 'all')->row_array(0);

                        $data['id'] = $this->babysitterId;
                        ...
                        $this->load->view('frontend/user/templates/offers/babysitters/edit-babysitter-details-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editDetailsSubmit(){
            if ($this->input->post('id')){
                ...
                echo $this->lang->line('user_edit_babysitter_details_success');
            }
        }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBabysitterRequest()){
                        ...
                        $data['id'] = $this->babysitterId;
                        $data['name'] = $details['name'];
                        $data['images'] = $this->CI->Offers_model->getGallery($this->babysitterId);

                        $this->load->view('frontend/user/templates/offers/babysitters/edit-babysitter-gallery-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function uploadify(){
            if (!empty($_FILES)){
                $this->babysitterId = $this->uri->segment(4);
                ...
                $images = $this->CI->Offers_model->getGallery($this->babysitterId);
                $query_data = array(
                    'offer_id' => $this->babysitterId,
                ...
                $this->db->where('id', $this->babysitterId);
        ...
        public function deleteImage(){
            if ($this->input->post('no')){
                $this->babysitterId = $this->input->post('babysitterId');
                $this->CI->Offers_model->deleteImage($this->babysitterId);
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBabysitterRequest()){
                        ...
                        $data['rooms'] = $this->CI->Babysitters_model->getRooms($this->babysitterId);

                        $this->load->view('frontend/user/templates/offers/babysitters/edit-babysitter-pricing-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editPricingSubmit(){
            if ($this->input->post('babysitterId')){
                $this->CI->Babysitters_model->editPricing();
                echo $this->lang->line('user_edit_babysitter_pricing_edit_pricing_success');
            }
        }
        ...
        public function addRoom(){
            if ($this->input->post('babysitterId')){
                $this->userId = $this->session->userdata('stbartsview-user');
                $this->babysitterId = $this->input->post('babysitterId');
                echo $this->CI->Babysitters_model->addRoom($this->babysitterId, $this->lang->line('user_edit_babysitter_pricing_add_room_name')).';;'.$this->lang->line('user_edit_babysitter_pricing_add_room_name').';;'.$this->lang->line('user_edit_babysitter_pricing_add_room_success');
            }
        }
        ...
        public function showRoom(){
            if ($this->input->post('roomId')){
                ...
                if ($this->validBabysitterRequest()){
                    ...
                    $room = $this->CI->Babysitters_model->getRoom($this->input->post('roomId'))->row_array(0);
                    ...
                    $this->load->view('frontend/user/forms/offers/babysitters/edit-babysitter-room-form', $data);
                }
                else{
                    $this->offerContent();
                }
        ...
        public function editRoomSubmit(){
            if ($this->input->post('roomId')){
                $this->CI->Babysitters_model->editRoom();
                echo $this->lang->line('user_edit_babysitter_pricing_edit_room_success');
            }
        }

        public function deleteRoom(){
            if ($this->input->post('roomId')){
                $this->CI->Babysitters_model->deleteRoom();
                echo $this->lang->line('user_edit_babysitter_pricing_edit_room_deleted');
            }
        }
        ...
        public function getPricing(){
            if ($this->session->userdata('stbartsview-user')){
                $result = '';
                $room = $this->CI->Babysitters_model->getRoom($this->uri->segment(4))->row_array(0);
        ...
        public function savePricing(){
            if ($this->input->post('dop_booking_calendar')){
                $this->CI->Babysitters_model->savePricing($this->uri->segment(4));
            }
        }

        private function validBabysitterRequest(){
            $this->db->where('id', $this->babysitterId);
