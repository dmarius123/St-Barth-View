...
/*
 * Title                   : St Barth View
 * File                    : application/controllers/user/boat.php
 * File Version            : 1.0
 * Author                  : Marius-Cristian Donea
 * Created / Last Modified : 03 July 2011
 * Last Modified By        : Marius-Cristian Donea
 * Description             : Login User - Boat Controller.
*/
    ...
    class Boat extends CI_Controller{
        ...
        public function add(){
            if ($this->session->userdata('stbartsview-user')){
                ...
                $data['user_add_offer_location_title'] = $this->lang->line('user_add_boat_location_title');
                $data['user_add_offer_location_info'] = $this->lang->line('user_add_boat_location_info');
                $data['user_add_offer_alternative_address'] = $this->lang->line('user_add_boat_alternative_address');
                $data['user_add_offer_details_title'] = $this->lang->line('user_add_boat_details_title');
                $data['user_offer_details_name'] = $this->lang->line('user_boat_details_name');
                $data['user_offer_details_description'] = $this->lang->line('user_boat_details_description');
                ...
                $this->load->view('frontend/user/templates/offers/add-boat-template', $data);
            }
            else{
                redirect('user/redirect');
            }
        ...
        public function validateName(){
            if ($this->input->post('name')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_boat_details_name_invalid').'</font>';
            }
        }
        ...
        public function validateDescription(){
            if ($this->input->post('description')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_boat_details_description_invalid').'</font>';
            }
        }
        ...
        public function item(){
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    $this->userId = $this->session->userdata('stbartsview-user');
                    $this->boatId = $this->input->post('id');

                    if ($this->validBoatRequest()){
                        $data = $this->lang->language;

                        $boat = $this->CI->Offers_model->getData($this->boatId, 'all')->row_array(0);

                        $boat['short_description'] = $this->CI->Functions_model->shortText($boat['description'], 150);
                        $boat['first_image'] = $this->CI->Offers_model->getFirstImage($boat['id']);
                        $boat['no_deals'] = $this->CI->Offers_model->getNoDeals($boat['id']);
                        $data['boat'] = $boat;


                        $this->load->view('frontend/user/templates/offers/boats/boat-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBoatRequest()){
                        ...
                        $details = $this->CI->Offers_model->getData($this->boatId, 'all')->row_array(0);
                        ...
                        $this->load->view('frontend/user/templates/offers/boats/edit-boat-details-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editDetailsSubmit(){
            if ($this->input->post('id')){
                ...
                echo $this->lang->line('user_edit_boat_details_success');
            }
        }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBoatRequest()){
                        ...
                        $data['id'] = $this->boatId;
                        $data['name'] = $details['name'];
                        $data['images'] = $this->CI->Offers_model->getGallery($this->boatId);

                        $this->load->view('frontend/user/templates/offers/boats/edit-boat-gallery-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function uploadify(){
            if (!empty($_FILES)){
                $this->boatId = $this->uri->segment(4);
                ...
                $images = $this->CI->Offers_model->getGallery($this->boatId);
                $query_data = array(
                    'offer_id' => $this->boatId,
        ...
        public function deleteImage(){
            if ($this->input->post('no')){
                $this->boatId = $this->input->post('boatId');
                $this->CI->Offers_model->deleteImage($this->boatId);
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validBoatRequest()){
                        ...
                        $data['rooms'] = $this->CI->Boats_model->getRooms($this->boatId);

                        $this->load->view('frontend/user/templates/offers/boats/edit-boat-pricing-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editPricingSubmit(){
            if ($this->input->post('boatId')){
                $this->CI->Boats_model->editPricing();
                echo $this->lang->line('user_edit_boat_pricing_edit_pricing_success');
            }
        }
        ...
        public function addRoom(){
            if ($this->input->post('boatId')){
                $this->userId = $this->session->userdata('stbartsview-user');
                $this->boatId = $this->input->post('boatId');
                echo $this->CI->Boats_model->addRoom($this->boatId, $this->lang->line('user_edit_boat_pricing_add_room_name')).';;'.$this->lang->line('user_edit_boat_pricing_add_room_name').';;'.$this->lang->line('user_edit_boat_pricing_add_room_success');
            }
        }
        ...
        public function showRoom(){
            if ($this->input->post('roomId')){
                ...
                if ($this->validBoatRequest()){
                    ...
                    $room = $this->CI->Boats_model->getRoom($this->input->post('roomId'))->row_array(0);
                    ...
                    $this->load->view('frontend/user/forms/offers/boats/edit-boat-room-form', $data);
                }
                else{
                    $this->offerContent();
                }
        ...
        public function editRoomSubmit(){
            if ($this->input->post('roomId')){
                $this->CI->Boats_model->editRoom();
                echo $this->lang->line('user_edit_boat_pricing_edit_room_success');
            }
        }

        public function deleteRoom(){
            if ($this->input->post('roomId')){
                $this->CI->Boats_model->deleteRoom();
                echo $this->lang->line('user_edit_boat_pricing_edit_room_deleted');
            }
        }
        ...
        public function getPricing(){
            if ($this->session->userdata('stbartsview-user')){
                $result = '';
                $room = $this->CI->Boats_model->getRoom($this->uri->segment(4))->row_array(0);
        ...
        public function savePricing(){
            if ($this->input->post('dop_booking_calendar')){
                $this->CI->Boats_model->savePricing($this->uri->segment(4));
            }
        }

        private function validBoatRequest(){
            $this->db->where('id', $this->boatId);
