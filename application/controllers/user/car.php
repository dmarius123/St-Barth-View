...
/*
 * Title                   : St Barth View
 * File                    : application/controllers/user/car.php
 * File Version            : 1.0
 * Author                  : Marius-Cristian Donea
 * Created / Last Modified : 03 July 2011
 * Last Modified By        : Marius-Cristian Donea
 * Description             : Login User - Car Controller.
*/
    ...
    class Car extends CI_Controller{
        ...
        public function add(){
            if ($this->session->userdata('stbartsview-user')){
                ...
                $data['user_add_offer_location_title'] = $this->lang->line('user_add_car_location_title');
                $data['user_add_offer_location_info'] = $this->lang->line('user_add_car_location_info');
                $data['user_add_offer_alternative_address'] = $this->lang->line('user_add_car_alternative_address');
                $data['user_add_offer_details_title'] = $this->lang->line('user_add_car_details_title');
                $data['user_offer_details_name'] = $this->lang->line('user_car_details_name');
                $data['user_offer_details_description'] = $this->lang->line('user_car_details_description');
                ...
                $this->load->view('frontend/user/templates/offers/add-car-template', $data);
            }
            else{
                redirect('user/redirect');
            }
        ...
        public function validateName(){
            if ($this->input->post('name')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_car_details_name_invalid').'</font>';
            }
        }
        ...
        public function validateDescription(){
            if ($this->input->post('description')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_car_details_description_invalid').'</font>';
            }
        }
        ...
        public function item(){
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    $this->userId = $this->session->userdata('stbartsview-user');
                    $this->carId = $this->input->post('id');

                    if ($this->validCarRequest()){
                        $data = $this->lang->language;

                        $car = $this->CI->Offers_model->getData($this->carId, 'all')->row_array(0);

                        $car['short_description'] = $this->CI->Functions_model->shortText($car['description'], 150);
                        $car['first_image'] = $this->CI->Offers_model->getFirstImage($car['id']);
                        $car['no_deals'] = $this->CI->Offers_model->getNoDeals($car['id']);
                        $data['car'] = $car;


                        $this->load->view('frontend/user/templates/offers/cars/car-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validCarRequest()){
                        ...
                        $details = $this->CI->Offers_model->getData($this->carId, 'all')->row_array(0);
                        ...
                        $this->load->view('frontend/user/templates/offers/cars/edit-car-details-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editDetailsSubmit(){
            if ($this->input->post('id')){
                ...
                echo $this->lang->line('user_edit_car_details_success');
            }
        }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validCarRequest()){
                        ...
                        $data['id'] = $this->carId;
                        $data['name'] = $details['name'];
                        $data['images'] = $this->CI->Offers_model->getGallery($this->carId);

                        $this->load->view('frontend/user/templates/offers/cars/edit-car-gallery-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function uploadify(){
            if (!empty($_FILES)){
                ...
                $images = $this->CI->Offers_model->getGallery($this->carId);
                $query_data = array(
                    'offer_id' => $this->carId,
        ...
        public function deleteImage(){
            if ($this->input->post('no')){
                $this->carId = $this->input->post('carId');
                $this->CI->Offers_model->deleteImage($this->carId);
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validCarRequest()){
                        ...
                        $data['rooms'] = $this->CI->Cars_model->getRooms($this->carId);

                        $this->load->view('frontend/user/templates/offers/cars/edit-car-pricing-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editPricingSubmit(){
            if ($this->input->post('carId')){
                $this->CI->Cars_model->editPricing();
                echo $this->lang->line('user_edit_car_pricing_edit_pricing_success');
            }
        }
        ...
        public function addRoom(){
            if ($this->input->post('carId')){
                $this->userId = $this->session->userdata('stbartsview-user');
                $this->carId = $this->input->post('carId');
                echo $this->CI->Cars_model->addRoom($this->carId, $this->lang->line('user_edit_car_pricing_add_room_name')).';;'.$this->lang->line('user_edit_car_pricing_add_room_name').';;'.$this->lang->line('user_edit_car_pricing_add_room_success');
            }
        }
        ...
        public function showRoom(){
            if ($this->input->post('roomId')){
                ...
                if ($this->validCarRequest()){
                    ...
                    $room = $this->CI->Cars_model->getRoom($this->input->post('roomId'))->row_array(0);
                    ...
                    $this->load->view('frontend/user/forms/offers/cars/edit-car-room-form', $data);
                }
                else{
                    $this->offerContent();
                }
        ...
        public function editRoomSubmit(){
            if ($this->input->post('roomId')){
                $this->CI->Cars_model->editRoom();
                echo $this->lang->line('user_edit_car_pricing_edit_room_success');
            }
        }

        public function deleteRoom(){
            if ($this->input->post('roomId')){
                $this->CI->Cars_model->deleteRoom();
                echo $this->lang->line('user_edit_car_pricing_edit_room_deleted');
            }
        }
        ...
        public function getPricing(){
            if ($this->session->userdata('stbartsview-user')){
                $result = '';
                $room = $this->CI->Cars_model->getRoom($this->uri->segment(4))->row_array(0);
        ...
        public function savePricing(){
            if ($this->input->post('dop_booking_calendar')){
                $this->CI->Cars_model->savePricing($this->uri->segment(4));
            }
        }

        private function validCarRequest(){
            $this->db->where('id', $this->carId);
