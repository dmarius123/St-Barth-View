...
/*
 * Title                   : St Barth View
 * File                    : application/controllers/user/chef.php
 * File Version            : 1.0
 * Author                  : Marius-Cristian Donea
 * Created / Last Modified : 03 July 2011
 * Last Modified By        : Marius-Cristian Donea
 * Description             : Login User - Chef Controller.
*/
    ...
    class Chef extends CI_Controller{
        ...
        public function add(){
            if ($this->session->userdata('stbartsview-user')){
                ...
                $data['user_add_offer_location_title'] = $this->lang->line('user_add_chef_location_title');
                $data['user_add_offer_location_info'] = $this->lang->line('user_add_chef_location_info');
                $data['user_add_offer_alternative_address'] = $this->lang->line('user_add_chef_alternative_address');
                $data['user_add_offer_details_title'] = $this->lang->line('user_add_chef_details_title');
                $data['user_offer_details_name'] = $this->lang->line('user_chef_details_name');
                $data['user_offer_details_description'] = $this->lang->line('user_chef_details_description');
                ...
                $this->load->view('frontend/user/templates/offers/add-chef-template', $data);
            }
            else{
                redirect('user/redirect');
            }
        ...
        public function validateName(){
            if ($this->input->post('name')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_chef_details_name_invalid').'</font>';
            }
        }
        ...
        public function validateDescription(){
            if ($this->input->post('description')){
                ...
                echo '<font style="color:'.$this->errorColor.';">'.$this->lang->line('user_chef_details_description_invalid').'</font>';
            }
        }
        ...
        public function item(){
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    $this->userId = $this->session->userdata('stbartsview-user');
                    $this->chefId = $this->input->post('id');

                    if ($this->validChefRequest()){
                        $data = $this->lang->language;

                        $chef = $this->CI->Offers_model->getData($this->chefId, 'all')->row_array(0);

                        $chef['short_description'] = $this->CI->Functions_model->shortText($chef['description'], 150);
                        $chef['first_image'] = $this->CI->Offers_model->getFirstImage($chef['id']);
                        $chef['no_deals'] = $this->CI->Offers_model->getNoDeals($chef['id']);
                        $data['chef'] = $chef;


                        $this->load->view('frontend/user/templates/offers/chefs/chef-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validChefRequest()){
                        ...
                        $details = $this->CI->Offers_model->getData($this->chefId, 'all')->row_array(0);
                        ...
                        $this->load->view('frontend/user/templates/offers/chefs/edit-chef-details-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editDetailsSubmit(){
            if ($this->input->post('id')){
                ...
                echo $this->lang->line('user_edit_chef_details_success');
            }
        }
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validChefRequest()){
                        ...
                        $data['id'] = $this->chefId;
                        $data['name'] = $details['name'];
                        $data['images'] = $this->CI->Offers_model->getGallery($this->chefId);

                        $this->load->view('frontend/user/templates/offers/chefs/edit-chef-gallery-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function uploadify(){
            if (!empty($_FILES)){
                $this->chefId = $this->uri->segment(4);
                ...
                $images = $this->CI->Offers_model->getGallery($this->chefId);
                $query_data = array(
                    'offer_id' => $this->chefId,
        ...
        public function deleteImage(){
            if ($this->input->post('no')){
                $this->chefId = $this->input->post('chefId');
                $this->CI->Offers_model->deleteImage($this->chefId);
            ...
            if ($this->session->userdata('stbartsview-user')){
                if ($this->input->post('id')){
                    ...
                    if ($this->validChefRequest()){
                        ...
                        $data['rooms'] = $this->CI->Chefs_model->getRooms($this->chefId);

                        $this->load->view('frontend/user/templates/offers/chefs/edit-chef-pricing-template', $data);
                    }
                    else{
                        $this->offerContent();
                    }
        ...
        public function editPricingSubmit(){
            if ($this->input->post('chefId')){
                $this->CI->Chefs_model->editPricing();
                echo $this->lang->line('user_edit_chef_pricing_edit_pricing_success');
            }
        }
        ...
        public function addRoom(){
            if ($this->input->post('chefId')){
                $this->userId = $this->session->userdata('stbartsview-user');
                $this->chefId = $this->input->post('chefId');
                echo $this->CI->Chefs_model->addRoom($this->chefId, $this->lang->line('user_edit_chef_pricing_add_room_name')).';;'.$this->lang->line('user_edit_chef_pricing_add_room_name').';;'.$this->lang->line('user_edit_chef_pricing_add_room_success');
            }
        }
        ...
        public function showRoom(){
            if ($this->input->post('roomId')){
                ...
                if ($this->validChefRequest()){
                    ...
                    $room = $this->CI->Chefs_model->getRoom($this->input->post('roomId'))->row_array(0);
                    ...
                    $this->load->view('frontend/user/forms/offers/chefs/edit-chef-room-form', $data);
                }
                else{
                    $this->offerContent();
                }
        ...
        public function editRoomSubmit(){
            if ($this->input->post('roomId')){
                $this->CI->Chefs_model->editRoom();
                echo $this->lang->line('user_edit_chef_pricing_edit_room_success');
            }
        }

        public function deleteRoom(){
            if ($this->input->post('roomId')){
                $this->CI->Chefs_model->deleteRoom();
                echo $this->lang->line('user_edit_chef_pricing_edit_room_deleted');
            }
        }
        ...
        public function getPricing(){
            if ($this->session->userdata('stbartsview-user')){
                $result = '';
                $room = $this->CI->Chefs_model->getRoom($this->uri->segment(4))->row_array(0);
        ...
        public function savePricing(){
            if ($this->input->post('dop_booking_calendar')){
                $this->CI->Chefs_model->savePricing($this->uri->segment(4));
            }
        }

        private function validChefRequest(){
            $this->db->where('id', $this->chefId);
