...
/*
 * Title                   : St Barth View
 * File                    : application/views/frontend/user/forms/offers/villas/edit-villa-details-form.php
 * File Version            : 1.0
 * Author                  : Marius-Cristian Donea
 * Created / Last Modified : 16 June 2011
 * Last Modified By        : Marius-Cristian Donea
 * Description             : Login User - Edit Villa Details form.
*/

?>
    <h3 id="edit-offer-details-title"><?=$user_edit_villa_details_title?></h3>
    <span class="separator"></span>
    <span id="edit-offer-details-info"></span>
    <form method="post" action="" onsubmit="return user_editVillaDetails()">
        <input type="hidden" name="villa_id" id="villa_id" class="user-area-input-style" value="<?=$id?>" />
        ...
        <div class="user-area-form-field">
            <label><?=$user_edit_villa_details_alternative_address?> <span class="required" id="info-address"><?=$user_optional_field?></span></label><br />
...
<?php
    ...
    foreach ($amenities->result() as $amenity):
        ...
        if ($checked){
            echo '<li><input type="checkbox" id="amenity'.$amenity->id.'" name="amenity'.$amenity->id.'" class="filter-checkbox" checked="checked" value="" />'.$general_amenities_villas[$amenity->index].'</li>';
        }
        else{
...
?>
        ...
        <input type="submit" name="submit" id="submit" class="user-area-submit-style" value="<?=$user_edit_villa_details_submit?>" />
